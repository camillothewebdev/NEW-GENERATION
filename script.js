...
window.onload = async () => {
  ...
  document.getElementById(
    "years-experience"
  ).textContent = `Expérience de la recherche/des enquêtes: ${
...
function updateControls() {
  ...
  if(newsList.length == 0 || newsList.length == 1)return;
  if (currentIndex == 0) {
    document.getElementById("controls-left").style.display = "none";
...
function showNewsRead() {
  ...
  newsList[currentNews].content.split('\n').map((line, index) => (
    line.trim() !== "" ? `<span>${line}<br /><br/></span>` : ''
  )).join('');
...
function generateCarouselItems(newsList) {
  if (newsList.length == 0) {
    document.getElementById(
      "news-controls"
    ).innerHTML = `<div style="height:40vh; width:100%; max-width:600px; display:flex; flex-direction:column; justify-content: center; margin:3em 0em; padding:0em 2em; background-color:#112e42; border-radius:20px"><h2 style="text-align : center">Aucune actualitée</h2></div>`;
  }else{
    document.getElementById(
      "news-controls"
    ).innerHTML =`<p id="number-of-news"></p>
    <img src="img/icon/left.png" class="controls-btn" id="controls-left" onclick="previews()" />
    <div class="carousel-container" id="my-carousel">
        <div class="carousel" id="carousel">
            
        </div>
    </div>
    <img src="img/icon/right.png" class="controls-btn" id="controls-right" onclick="next()" />`
  }
  if(newsList.length > 0){
    ...
    .map((news) => {
      return `  
          <div class="carousel-item" style= "background-image : url('${news.image}')" onclick="showNewsRead()">
            <h2>${news.title}</h2>
            <button tabindex="-1" onclick="showNewsRead()">Lire la suite</button>
          </div>
        `;
    })
    .join("");
...
function generateOtherNewsItems(newsList) {
  if (newsList.length <= 1) {
    document.getElementById("other-news-list").innerHTML =
      "<p style='margin:auto'> Aucune autre actualitée </p>";
    return;
  }
  document.getElementById(
    "other-news-list"
  ...
  document.getElementById("other-news-list").innerHTML += newsList
    .map((news,index) => {
      if (news.id != newsList[currentNews].id) {
        return `
            <div class="other-news" style= "background-image : url('${news.image}')"  onclick="readOtherNews(${index})">
              <h2>${news.title}</h2>
              <button onclick="readOtherNews(${index})">Lire la suite</button>
            </div>
          `;
      }
    })
...
async function addNews() {
  if (
    ...
    showMessageDialog("Veuillez insérer tous les informations !");
...
const date = new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' });
  ...
  await getNews()
  closeWaitDialog();
  showMessageDialog("Enregistrement effectué !");
  document.getElementById("btn-ok").name = "new-news"
}
async function deleteNews() {
  ...
  await getNews();
  closeWaitDialog();
  showMessageDialog("Suppression effectuée !");
  document.getElementById("btn-ok").name = "deleted-news";
}
async function editNews() {
  ...
  await getNews()
  closeWaitDialog();
  showMessageDialog("Modification effectuée !");
  document.getElementById("btn-ok").name = "edited-news"
}
function sendMessage() {
  if (
    ...
    showMessageDialog("Veuillez entrer l'email et le message !");
    document.getElementById("btn-ok").name = "";
    return;
  }
  let name = document.getElementById("contact-name").value;
  let message = document.getElementById("contact-message").value;
  let subject = "J'ai visité votre site web";
...
function showMessageDialog(message) {
  ...
  document.getElementById("dialog-message").style.opacity = '1'; 
  document.getElementById("dialog-message").style.transform = 'scale(1)'; 
}
function closeMessageDialog(refresh) {
  if(document.getElementById("btn-ok").name == "go-to-news"){
    location.href = "#news"
  }
  ...
  document.getElementById("dialog-message").style.opacity = '0'; 
  document.getElementById("dialog-message").style.transform = 'scale(0.7)'; 
  if(document.getElementById("btn-ok").name == "new-news"){
    currentIndex = 0;
    updateControls();
    updateNumberOfNews();
    document.getElementById("carousel").style.transform = `translateX(0%)`;
  }
  if( document.getElementById("btn-ok").name == "deleted-news"){
    currentIndex = 0;
    updateControls();
    updateNumberOfNews();
    document.getElementById("carousel").style.transform = `translateX(0%)`;
  }
  if(document.getElementById("btn-ok").name == "edited-news"){
    updateControls();
    updateNumberOfNews();
    document.getElementById("carousel").style.transform = `translateX(-${
      currentNews * 100
...
function showWaitDialog() {
  ...
  document.getElementById("dialog-wait").style.opacity = '1'; 
  document.getElementById("dialog-wait").style.transform = 'scale(1)'; 
}
function closeWaitDialog(refresh) {
  document.getElementById("dialog-wait").close();
  document.getElementById("dialog-wait").style.opacity = '0'; 
  document.getElementById("dialog-wait").style.transform = 'scale(0.7)'; 
  document.body.style.overflow = "";
}
